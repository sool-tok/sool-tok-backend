...
const rooms = {
  // [roomId] : { _id, title, maxNum, memberList[], isLocked, filter }
};

const gameMembers = {};

const phrases = ['삼겹살', '소주', '막걸리', '곱창', '찜닭'];
...
io.on('connection', socket => {
  ...
  socket.on('filter', ({ roomId, isFilterOn, filter }) => {
    if (!roomId) return;

    rooms[roomId].filter = isFilterOn ? filter : null;
    io.to(roomId).emit('filter', { isFilterOn, filter });
  });
  ...
  socket.on('start game', ({ title, roomId }) => {
    ...
    if (title === 'speechBomb') {
      const explosionTime = _.random(10, 15) * 1000;
  ...
  socket.on('turn change', status => {
    const { roomId } = status;
    const currentIndex = gameMembers[roomId].findIndex(socketId => socketId === socket.id);
